...
export interface MapProvider {
  ...
  reasons: string[];
  pinType: 'verified' | 'popular' | 'mobile' | 'standard';
}
...
export class MapService {
  ...
  async getDirectMode(query: { providerId: string; lat: number; lng: number }): Promise<any> {
    ...
    // Determine pin type
    let pinType: 'verified' | 'popular' | 'mobile' | 'standard' = 'standard';
    if (org.isVerified) pinType = 'verified';
    if (org.isPopular) pinType = 'popular';
    if (org.isMobile) pinType = 'mobile';
